...
std::shared_ptr<MeshStatic> MeshStatic::from_toml(const FileInfo& toml_path)
{
  ...
  std::shared_ptr<MeshStatic> staticmesh(new MeshStatic());
...
std::vector<Rect> MeshStatic::bbox() const
{
...
void MeshStatic::think(std::shared_ptr<Game>& game)
{
  auto& pos = position();
  sprite->x = pos.x;
  sprite->y = pos.y;
...
void MeshStatic::render(Renderer* renderer)
{
  sprite->render(renderer);
}

void MeshStatic::serialize(std::vector<NetField>& list)
{
  NetFieldType cls = NetFieldType::MeshStatic;

  // Stop looking at me like that.
  // This macro helps expand our fields into
  // field type, offset into the class, sizeof object, and data
  #define SNF(field) NetFieldMacro(MeshStatic, field)
...
bool MeshStatic::deserialize(const std::vector<NetField>& fields)
{
  return true;
}
} // namespace raptr
