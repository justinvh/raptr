...
void Game::handle_controller_event(const ControllerEvent& controller_event)
{
    ...
    bool is_start = ((e.type == SDL_CONTROLLERBUTTONDOWN && e.cbutton.button == SDL_CONTROLLER_BUTTON_START) || (e.type == SDL_KEYDOWN && e.key.keysym.scancode == SDL_SCANCODE_RETURN));
    ...
    Entity* entity, const Rect& bbox, IntersectEntityFilter post_filter, size_t limit)
{
    ...
    rtree.Search(
        min_bounds, max_bounds, [](Entity* found, void* context) -> bool {
...
bool Game::poll_events()
{
    ...
    bool is_joystick = (e.type == SDL_JOYAXISMOTION || e.type == SDL_JOYBUTTONDOWN || e.type == SDL_JOYBUTTONUP);

    bool is_controller = (e.type == SDL_CONTROLLERAXISMOTION || e.type == SDL_CONTROLLERBUTTONDOWN || e.type == SDL_CONTROLLERBUTTONUP);
...
bool Game::process_engine_events()
{
    ...
    for (auto& entity : entities) {
        ...
        if (std::fabs(old_point.x - new_point.x) > 0.5 || std::fabs(old_point.y - new_point.y) > 0.5) {
